@extends('layouts.master')
    ...
    @section('title' , 'Admin|Rebuilt Tyre Disposal, Edit')
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 ">
                        <div class="x_panel">
                            ...
                            <div class="x_content">
                                <br/>
                                <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" method="post" action="{{route('admin.rebuilt.tyre.disposal.update')}}" enctype="multipart/form-data">
                                    @csrf
                                    
                                <input type="hidden" name="id" value="{{$issue->id}}">  
                                <h6>Rebuilt Tyre Disposal, Edit</h6>
                                <hr>    
                                ...
                                <div class="row">
                                    <div class="form-group col-lg-4">
                                        ...
                                        <select class="form-control select2 @error('tyre_code') is-invalid @enderror" id="tyre_code" data-placeholder="Select Tyre Code" name ="tyre_code" style="width: 100%;">
                                            <option value="" >Select Tyre Code</option>
                                            @if(count($stockCodes) > 0)
                                                @foreach($stockCodes as $v)
                                                <option value="{{$v->id}}" @if($issue->tyre_code == $v->id) selected @endif>{{$v->item_id}}</option>
                                                @endforeach
                                            @endif
                                        </select>  
                                        @error('item_code')
                                    ...
                                    <div class="form-group col-lg-4">
                                        ...
                                        <input type="number" class="form-control  @error('rate') is-invalid @enderror" value="{{$issue->rate}}" id="rate" name="rate" placeholder="Rate">
                                        @error('rate')
                                        <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                        </span>
